...
function validarDocumento () {
 url = 'https://spreadsheets.google.com/feeds/list/1Bym2WkmBL9u2OUm7Cc-3GeZkKlfVE_CREGA6uLbSkNo/1/public/values?alt=json'
 ...
 if (this.status == 500) { sinResultados (); }
  ...
  $("#resultados").html("<b>¡Parece que hemos encontrado algo!…</b>");
  ...
  if (datos.feed.entry[i]["gsx$documento"].$t == $("#documento").val()) {
   ...
   '<a href=' + datos.feed.entry[i]["gsx$mergeddocurl-generadorprimero"].$t + ' target="_blank"><strong>' + datos.feed.entry[i]["gsx$taller"].$t + ', </br>¡clic aquí para descargar!</strong></a>' + '</li></div>');
   //i = 0;
  } i--; } } }
